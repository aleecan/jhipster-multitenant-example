package com.mikado.mikadosaas.config.multitenant;

import java.util.Collection;
import java.util.List;
import java.util.stream.Collectors;

import com.mikado.mikadosaas.config.DatabaseConfiguration;
import com.mikado.mikadosaas.domain.DataSourceConfig;
import com.mikado.mikadosaas.repository.DataSourceConfigRepository;
...
public class MultiTenantDataSourceLookup extends MapDataSourceLookup {
    ...
    public void handleContextRefresh(ContextRefreshedEvent event) throws LiquibaseException, Exception {
        ...
        List<DataSourceConfig> dataSourceConfigs = configRepository.findAll();
        addTenantDataSources(dataSourceConfigs);
    }
    ...
    private void liquibaseUpdate(HikariDataSource customDataSource) throws Exception {
        ...
        if (environment.acceptsProfiles(Profiles.of(JHipsterConstants.SPRING_PROFILE_NO_LIQUIBASE))) {
            ...
            log.debug("Configuring Liquibase mikadosaas mode");
        }
